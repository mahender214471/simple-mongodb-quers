...
const GenerateMethods = ( Model ) => {
     try{
         ...
         methods.creadeDoc = async(documents) => {
             try{
                if(!Array.isArray(documents)){
                   throw { Error:"Documents create failed" , message:"Documents should we an array"}
                }
                for (let i = 0; i < documents.length; i++) {
                    if(typeof(documents[i]) != 'object'){
                        throw { Error:"Documents create failed" , message:"Each document in array should we an object"}
                    }
                    else{
         ...
         // !Find documents
         methods.finddocs = async ( filter , projection , limit , skip , sort , populate) => {
                  return await Model.find(filter , projection).limit(limit).skip(skip).sort(sort).populate(populate);   
         }
         ...
         methods.findByIdAndUpdate = async( id , dataToUpdate) => {
                return await Model.findByIdAndUpdate(id , {...dataToUpdate , updatedAt:moment().valueOf() });
         }
         // ! Updates documents 
         methods.updateOne = async (filter , dataToUpdate ) => {
              await Model.updateOne(filter , {...dataToUpdate , updatedAt:moment().valueOf() });
              return await methods.finddocs(filter);
         }
         methods.updateMany = async (filter , dataToUpdate ) => {
            await Model.updateMany(filter , {...dataToUpdate , updatedAt:moment().valueOf() });
            return await methods.finddocs(filter);
       }
       // ! Delete documets
